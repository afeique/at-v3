...
<?php
			if($ipbwi->member->isLoggedIn() && ($ipbwi->member->isAdmin() || $ipbwi->member->isSuperMod() || ($post['author_id'] === $userInfo['member_id']))){
				$ipbwi->report->create($post['pid'],'exampletest');
				$reports = $ipbwi->report->getList($post['pid']);
				
				echo intval($reports['general']['num_reports']).' reports | <a href="post_edit.php?post_id='.$post['pid'].'">edit</a> | <a onclick="if(confirm(\'Really delete this post?\')){return true;}else{return false;}" href="post_delete.php?post_id='.$post['pid'].'">delete</a>';
			}
?>
		</div>
	</div>
