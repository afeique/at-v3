	...
	if($ipbwi->gallery->installed){
		...
		if($gallery){
			...
			foreach($gallery as $img){
				echo '
					<ul>
						<li><a href="'.$ipbwi->getBoardVar('url').'gallery/image/'.$img['image_image_id'].'-'.$img['image_caption_seo'].'/"><img src="'.$ipbwi->gallery->url.$img['image_directory'].'/tn_'.$img['image_masked_file_name'].'" alt="'.$img['image_description'].'" title="'.$img['image_description'].'" /></a></li>
					</ul>
				';
			}
		}
