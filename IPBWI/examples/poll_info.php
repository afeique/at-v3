...
?>
		...
		<p><strong><?php echo  $ipbwi->bbcode->bbcode2html($pollTopic['description']); ?></strong> created from <?php echo $ipbwi->member->id2displayname($pollTopic['author_id']); ?> @ <?php echo $ipbwi->date($pollTopic['start_date']); ?></p>
		...
		foreach($poll['choices'] as $key => $choice){
			...
			foreach($choice as $option){
				...
				// print poll result
					...
					echo '<tr><td style="width:50%;">'.$option['option_title'].'</td><td style="width:25%;">[<strong>'.$option['votes'].'</strong>]</td><td style="width:25%;"><img src="'.$ipbwi->getBoardVar('img_url').'/gradient_bg.png" width="'.$option['percentage'].'" height="11" alt="" style="background-color:#243F5C;" />&nbsp;['.$option['percentage'].'%]</td></tr>'."\n";
				}
				$i++;
			}
			echo "\n\n";
