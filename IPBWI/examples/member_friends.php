...
echo '
			<tr><th colspan="3">'.$ipbwi->member->id2displayname($friend['friends_friend_id']).'</th></tr>
			<tr>
				<td>'.$ipbwi->member->photo($friend['friends_friend_id']).'</td>
				<td>'.$ipbwi->date($friend['friends_added']).'</td>
				<td><form method="post"><input type="hidden" name="friends_del_id" value="'.$friend['friends_friend_id'].'" /><input type="submit" name="remove_friend" value="Now!" /></form></td>
			</tr>
';
		}
?>
