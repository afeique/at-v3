 ...
 * Last Updated: $Date: 2013-02-05 06:22:45 -0500 (Tue, 05 Feb 2013) $
 * </pre>
 *
...
class moo
{
	...
	function __construct( ipsRegistry $registry )
	{
		...
		$this->newParser    = new $classToLoad();
		...
		if ( $this->end == 0 OR ( $this->end > $this->processed ) )
		{
			...
			while( $r = $this->DB->fetch( $outer ) )
			{
				...
				switch( $type )
				{
					...
					case 'posts':
						$allowHtml = ( $r['post_htmlstate'] && $r['use_html'] && $this->caches['group_cache'][ $r['member_group_id'] ]['g_dohtml'] );
						
						$this->newParser->set( array( 'parseArea'      => 'topics',
													  'memberData'     => $r,
													  'parseBBCode'    => $r['use_ibc'],
													  'parseHtml'      => $allowHtml,
